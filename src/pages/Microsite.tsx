...
const Microsite = () => {
  ...
  // Render content based on service type
    ...
    <>
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold mb-1">شركة الشحن</p>
            <p className="text-muted-foreground text-sm">{payload.carrier?.name}</p>
          </div>
        </div>
      ...
      <div className="space-y-6 mb-8">
        <div className="bg-secondary/30 p-4 rounded-lg">
          <h3 className="font-bold mb-3">المرسل</h3>
          <div className="space-y-2 text-sm">
            <p><span className="font-semibold">الاسم:</span> {payload.sender?.name}</p>
            <p><span className="font-semibold">الهاتف:</span> {payload.sender?.phone}</p>
            <p><span className="font-semibold">العنوان:</span> {payload.sender?.address}</p>
          </div>
        </div>
        
        <div className="bg-secondary/30 p-4 rounded-lg">
          <h3 className="font-bold mb-3">المستلم</h3>
          <div className="space-y-2 text-sm">
            <p><span className="font-semibold">الاسم:</span> {payload.recipient?.name}</p>
            <p><span className="font-semibold">الهاتف:</span> {payload.recipient?.phone}</p>
            <p><span className="font-semibold">العنوان:</span> {payload.recipient?.address}</p>
          </div>
        </div>
        
        <div className="bg-secondary/30 p-4 rounded-lg">
          <h3 className="font-bold mb-3">تفاصيل الشحنة</h3>
          <div className="space-y-2 text-sm">
            <p><span className="font-semibold">الوصف:</span> {payload.package?.description}</p>
            <p><span className="font-semibold">الوزن:</span> {payload.package?.weight} كجم</p>
            <p><span className="font-semibold">الأبعاد:</span> {payload.package?.dimensions}</p>
          </div>
        </div>
      </div>
      
      <div className="bg-gradient-primary p-6 rounded-xl text-primary-foreground mb-6">
        <p className="text-sm mb-2 opacity-90">تكلفة الشحن</p>
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="overflow-hidden shadow-elevated">
            ...
            <div className="p-8">
              {type === 'shipping' && renderShippingContent()}
              {type === 'chalet' && renderChaletContent()}
